...
if (len(sys.argv) != numargs):
    ...
    sc_numrepsw[0] = sc_par * numrepsw[0]
    sc_numrepsall[0] = (sc_par +1) * numrepsall[0]
    sc_bulkrepsall[0] = (sc_par) * numrepsall[0]

    #offset
    #setting white offset sc_par + 2 cells away (sc_par * black + 2 for the gb)
    offsetw = np.array([(sc_par+2)* numrepsb[0],0,0])
    #offset for the b to w gb = sc_par
    offsetbw = np.array([(sc_par)* numrepsb[0],0,0])
    #offset for the w to b gb = 2* sc_par + 2
    offsetwb = np.array([(2*sc_par+2)* numrepsb[0],0,0])
